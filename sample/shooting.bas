...
'X: SPRITE ID
'E,F: PRINT LOCATION
'A: STICK VALUE, P: PREVIOUS STICK (FOR EDGE DETECT)
'G: SHOT FLAG
'K: SCORE
'C,D: SHOT LOCATION
...
20 PLAY "T1CDEFAB1O5"
30 PRINT "SCORE:";K
100 E=14:F=12:C=0:D=0:P=0
110 A=STICK(0)
111 IF E<>C OR F<>D THEN LOCATE C,D:PRINT " ":LOCATE E,F:PRINT CHR$(&HDD):C=E:D=F
115 IF STRIG(0)=8 THEN 130
117 'MOVE AIM ONLY ON RISING EDGE (ONE STEP PER PRESS)
120 E=E+((A AND 1)=1 AND (P AND 1)=0)*(E<27)-((A AND 2)=2 AND (P AND 2)=0)*(E>0):F=F+((A AND 4)=4 AND (P AND 4)=0)*(F<23)-((A AND 8)=8 AND (P AND 8)=0)*(F>0):P=A:GOTO 180
125 'IF NOT SHOT, GOTO 200 FOR REST OF LOGIC
130 PLAY "B":LOCATE E,F:PRINT "*";:PAUSE 10:LOCATE E,F:PRINT " ":LOCATE E,F:PRINT CHR$(&HDD);
...
182 NEXT
190 IF K<(5-S)*1000 THEN PAUSE 1:GOTO 110
